...
import de.jamala.train_announcer.network.SetAnnouncerDataPayload
...
class AnnouncerScreen(
    ...
    override fun init() {
        ...
        genBtn = addRenderableWidget(Button.builder(Component.literal("Generate & Preview")) { _ ->
            de.jamala.train_announcer.client.ClientHooks.testAL()
            val text = textBox.value
            if (text.isNotBlank()) {
                AudioPlayer.play(null, text, currentVoice, currentLanguage, currentSpeed, currentVolume, currentReverb, currentMaxRange)
            }
        ...
        tabBtn = addRenderableWidget(Button.builder(Component.literal("Tab: Main")) { _ ->
            ...
            updateVisibility()
        }.bounds(centerX - 100, centerY + 65, 95, 20).build())

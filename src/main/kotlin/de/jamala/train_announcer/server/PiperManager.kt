package de.jamala.train_announcer.server
...
object PiperManager {
    ...
    fun getPiperDirectory(): File {
        ...
        val dir = File(serverDir, "train_announcer_piper")
    ...
    suspend fun downloadModel(language: String, voice: String, onnxUrl: String, jsonUrl: String) {
        withContext(Dispatchers.IO) {
            try {
                ...
                TrainAnnouncer.LOGGER.info("Downloaded Piper model $language-$voice")
            } catch (e: Exception) {
                TrainAnnouncer.LOGGER.error("Failed to download model $language-$voice", e)
            }
        }
    }
    ...
    suspend fun setupPiper() {
        withContext(Dispatchers.IO) {
            ...
            if (!piperDir.exists() || (piperDir.list()?.size ?: 0) < 5) {
                piperDir.mkdirs()
                TrainAnnouncer.LOGGER.info("Extracting bundled Piper TTS binaries...")
                
                try {
                    val modFile = ModList.get().getModFileById(TrainAnnouncer.ID).file
                    ...
                    if (Files.exists(resourcePath)) {
                        ...
                        TrainAnnouncer.LOGGER.info("Successfully extracted Piper TTS.")
                    } else {
                        TrainAnnouncer.LOGGER.error("Bundled Piper TTS not found in jar!")
                    }
                } catch (e: Exception) {
                    TrainAnnouncer.LOGGER.error("Failed to extract Piper TTS", e)
                }
            }
            ...
            if (!piperFile.exists()) {
                TrainAnnouncer.LOGGER.warn("Piper executable not found at ${piperFile.absolutePath}. Local TTS will fail unless Piper is installed.")
            }
        }
    }
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (onnxFile == null || jsonFile == null) {
                    TrainAnnouncer.LOGGER.error("Piper model for $language-$voice not found in ${modelsDir.absolutePath}")
                ...
                if (!piperFile.exists()) {
                    TrainAnnouncer.LOGGER.error("Piper executable not found at ${piperFile.absolutePath}")
                    return@withContext null
                }
                ...
                if (exitCode != 0) {
                    ...
                    TrainAnnouncer.LOGGER.error("Piper process failed with code $exitCode: $output")
                    outputFile.delete()
                    return@withContext null
                }
                ...
                TrainAnnouncer.LOGGER.error("Error generating Piper audio", e)
                return@withContext null
            }
        }
    }
